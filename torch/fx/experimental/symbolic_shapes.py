...
class EqualityConstraint(Constraint):
    ...
    def is_derived(self, source1, source2, symbol, expr):
        return any(
            src1 == source1 and src2 == source2 and fn(symbol) == expr
            for src1, src2, fn in self.derived_equalities
        )
...
class DimConstraints:
    ...
    def solve_supported_equivalence(cls, expr, solution):
        ...
        if isinstance(expr, sympy.Symbol):
            if isinstance(solution, int) or solution.is_integer:
                return solution
            else:
    ...
    def prettify_results(
        ...
        if self._dcp.source_name_to_debug_name:
            ...
            for s in self._static_results.union(self._dynamic_results):
                ...
                if t == s:
                    continue
                left, op, right, *others = t.split(" ")
                # TODO(avik): fix
                if op == "==" and left == right and not others:
                    continue
                if right.isdigit():
                    ...
                    # TODO(avik): fix
                    if others:
                        results[left]["eq"] = " ".join([right, *others])
                    else:
        ...
        *,
        ...
        if equalities_inputs:
            ...
            def get_symbol(tensor_dim_src):
                if isinstance(tensor_dim_src, sympy.Expr):
                    return tensor_dim_src
            ...
            for src1, src2, fn in equalities_inputs.derived_equalities:
                s1, s2 = get_symbol(src1), get_symbol(src2)
                s2_ = fn(s2)
                concrete_val = self.evaluate_expr(sympy.Eq(s1, s2_))
                if not concrete_val:
                    # TODO(avik): src may be a symbol
                    raise ConstraintViolationError(
                        f"Expected input {src1.name()} to be equal to "
                        f"{s2_.subs({s2: sympy.Symbol(self.debug_name(src2))})}, "
                        f"where {self.debug_name(src2)} = {s2.subs(self.var_to_val)}, "
            ...
            for source, expr in input_guards:
                ...
                if (
                    ...
                    if (
                        ...
                        not equalities_inputs.is_derived(source, symbol_to_source[symbol][0], symbol, expr)
                    ):
                        msg = (
                            f"The values of {self.debug_name(source)} = {source.name()} must always be related to the values of "
                            f"{self.debug_name(symbol_to_source[symbol][0])} = {symbol_to_source[symbol][0].name()} by "
                            f"{self.debug_name(source)} = {expr.subs({symbol: sympy.Symbol(self.debug_name(symbol_to_source[symbol][0]))})}."
                        )
                        record_constraint_violation(equalities_inputs.warn_only, self.debug_name(source), msg)
